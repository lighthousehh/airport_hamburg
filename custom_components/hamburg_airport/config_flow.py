    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            if await self._test_api_key(user_input[CONF_API_KEY]):
                ...
                return self.async_create_entry(title="Hamburg Airport", data=user_input)
    ...
    async def async_step_init(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Required(CONF_SCAN_INTERVAL, default=opts.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)):
                    vol.All(vol.Coerce(int), vol.Range(min=1, max=60)),
                vol.Required(CONF_PAST_COUNT, default=opts.get(CONF_PAST_COUNT, DEFAULT_PAST_COUNT)):
                    vol.All(vol.Coerce(int), vol.Range(min=0, max=10)),
                vol.Required(CONF_FUTURE_COUNT, default=opts.get(CONF_FUTURE_COUNT, DEFAULT_FUTURE_COUNT)):
